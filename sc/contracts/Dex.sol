...
contract Dex {
    ...
    function buy(address _router, address _token, uint256 _amount, bool isAmountIn) internal returns (uint256) {
        ...
        if (isAmountIn == true) {
            ...
            console.log("amount", balanceAfter - balanceBefore);
            console.log("Buy amount", _amount);
            return amountOut;
        } else {
            ...
            router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountIn}(
                _amount, path, address(this), block.timestamp + 1000
            );
            return _amount;
        }
    }
    ...
    function sell(address _router, address _token, uint256 _amount) internal returns (uint256) {
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            _amount, amountOut, path, address(this), block.timestamp + 1000
        );
    ...
    function volumeEachBuyThenSell(address _router, address _token, uint256 _amountEachLoop, uint256 numLoops)
        public
        payable
    {
        IUniswapV2Router02 router = IUniswapV2Router02(_router);
    ...
    function volumeAllBuyThenSell(
        address _router,
        address _token,
        uint256 _totalAmountBuy,
        uint256 numLoopsBuy,
        uint256 numLoopsSell
    ) public payable {
    ...
    function volumeRandomBuyAndSell(address _router, address _token, uint256 _totalAmountBuy, uint256 _numLoopsBuy)
        public
        payable
    {
